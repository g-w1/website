...
async function getBestBuzz({ username, difficulties, setName, includeMultiplayer, includeSingleplayer, startDate, endDate }) {
    ...
    const data = await tossupData.findOne(matchDocument, { sort: { celerity: -1 } });
    if (!data)
        return null;
...
async function getUser(username, showPassword = false) {
    return await users.findOne({ username: username }, { projection: { password: showPassword ? 1 : 0 } });
}
...
async function verifyEmail(user_id) {
    return await users.updateOne({ _id: new ObjectId(user_id) }, { $set: { verifiedEmail: true } });
}
export { createUser, getBestBuzz, getCategoryStats, getSingleBonusStats, getSingleTossupStats, getBonusGraphStats, getTossupGraphStats, getSubcategoryStats, getUser, getUsername, getUserField, getUserId, isAdmin, isAdminById, recordBonusData, recordTossupData, updateUser, verifyEmail, };
