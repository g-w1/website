 ...
 */
async function getQuery({ queryString, difficulties, setName, searchType = 'all', questionType = 'all', categories, subcategories, maxReturnLength, randomize = false, regex = false, exactPhrase = false, ignoreDiacritics = false, powermarkOnly = false, tossupPagination = 1, bonusPagination = 1, minYear, maxYear, verbose = false, } = {}) {
    if (verbose)
 ...
 */
async function getRandomTossups({ difficulties = DIFFICULTIES, categories = CATEGORIES, subcategories = SUBCATEGORIES_FLATTENED, number = 1, minYear = DEFAULT_MIN_YEAR, maxYear = DEFAULT_MAX_YEAR, powermarkOnly = false, } = {}) {
    const aggregation = [
 ...
 * @returns {Promise<Array<JSON>>}
 */
async function getRandomBonuses({ difficulties = DIFFICULTIES, categories = CATEGORIES, subcategories = SUBCATEGORIES_FLATTENED, number = 1, minYear = DEFAULT_MIN_YEAR, maxYear = DEFAULT_MAX_YEAR, bonusLength, } = {}) {
    const aggregation = [
        { $match: { 'set.year': { $gte: minYear, $lte: maxYear } } },
        { $sample: { size: number } },
        { $project: { reports: 0 } },
...
export { closeDatabase, connectToDatabase, getBonusById, getNumPackets, getPacket, getQuery, getRandomName, getRandomTossups, getRandomBonuses, getReports, getSet, getSetId, getSetList, getTossupById, reportQuestion, updateSubcategory, };
