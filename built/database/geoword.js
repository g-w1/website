...
async function getPacket(packetName, division) {
    return await tossups.find({ 'packet.name': packetName, division }, { sort: { questionNumber: 1 } }).toArray();
}
async function getPacketList() {
    return await packets.find({ test: { $ne: true } }, {
        sort: { order: 1 },
...
async function getProtests(packetName, division) {
    const protests = await buzzes.find({ 'packet.name': packetName, division, pendingProtest: { $exists: true } }, { sort: { questionNumber: 1 } }).toArray();
    const packet = await tossups.find({ 'packet.name': packetName, division }, { sort: { questionNumber: 1 } }).toArray();
...
async function recordDivision(packetName, division, user_id) {
    const packet = await packets.findOne({ name: packetName });
    return await divisionChoices.replaceOne({ user_id, 'packet.name': packetName }, { user_id, packet: { _id: packet._id, name: packetName }, division }, { upsert: true });
}
async function recordPayment(packetName, user_id) {
    const packet = await packets.findOne({ name: packetName });
    return await payments.replaceOne({ user_id, 'packet.name': packetName }, { user_id, packet: { _id: packet._id, name: packetName }, createdAt: new Date() }, { upsert: true });
}
async function recordProtest(packetName, questionNumber, user_id) {
    return await buzzes.updateOne({ 'packet.name': packetName, questionNumber, user_id }, { $set: { pendingProtest: true } });
}
async function resolveProtest(buzz_id, decision, reason) {
    ...
    return await buzzes.updateOne({ _id: buzz_id }, { $set: updateDocument });
}
export { checkPayment, getAdminStats, getAnswer, getAudio, getBuzzCount, getBuzzes, getCost, getDivisionChoice, getDivisions, getLeaderboard, getPacket, getPacketList, getPacketStatus, getPlayerList, getProgress, getProtests, getQuestionCount, getUserStats, recordBuzz, recordDivision, recordPayment, recordProtest, resolveProtest, };
